...
<!-- TODO(AS4): Replace link when there is an expressMiddleware article-->

> Subscriptions are **not** supported by Apollo Server 4's `startStandaloneServer` function. To enable subscriptions, you must first [swap to using the `expressMiddleware` function](./migration#expressmiddleware) (or any other Apollo Server integration package that supports subscriptions).
>
...
<!-- TODO(AS4) swap out link once we have a expressMiddleware func article -->

> The following steps assume you've already [swapped to using the `expressMiddleware` function](./migration#migrating-from-apollo-server-express).
